...
export default class TaskApp extends React.Component {
  ...
  componentDidMount() {
    /*
    this.setState({toDoList: tasksData.map(task => 
  ...
  changePriority(idTask, newPriority) {
    ...
    const updatedTask = copyToDoList.find(task => task.id === idTask);
  ...
  addTask(desc, duration) {
    ...
    this.setState(prevState => ({toDoList: [...prevState.toDoList, newTask]}), this.saveToDoList);
  }
